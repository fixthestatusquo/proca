...
export default class CampaignTranslate extends Command {
  static description = "Translate a campaign's locale file from one language to another.";
  ...
  static flags = {
    ...
    namespace: Flags.string({
        description: "Limit translation to a specific namespace (e.g., widget, form)",
        required: false
    }),
  ...
  async translate(text, from, to) {
    ...
    if (!accountId || !authToken) {
      this.error("Both CF_ACCOUNT and CF_TOKEN environment variables must be set.");
    }

    // Using the translation-specific model @cf/meta/m2m100-1.2b as it's best suited for this task.
    const model = "@cf/meta/m2m100-1.2b";
    const url = `https://api.cloudflare.com/client/v4/accounts/${accountId}/ai/run/${model}`;

    this.log(`Translating via Cloudflare AI...`);
    ...
    if (!response.ok) {
      ...
      this.error(`API call failed with status ${response.status}: ${errorText}`);
    }
...
console.log({
  ...
  async traverseAndTranslate(source, target, { from, to, force, path = [] }) {
    for (const key of Object.keys(source)) {
      ...
      if (typeof sourceValue === "string" && sourceValue.trim() !== '') {
        ...
        try {
          const translatedText = await this.translate(sourceValue, from, to);
          this.setValue(target, [key], translatedText);
          this.log(`${currentPath.join(".")}: ${translatedText}`);
        } catch (error) {
          this.error(`Error translating ${currentPath.join(".")}: ${error.message}`);
        }
      } else if (typeof sourceValue === "object" && sourceValue !== null) {
        if (!this.getValue(target, [key])) {
          this.setValue(target, [key], {});
        }
        await this.traverseAndTranslate(sourceValue, this.getValue(target, [key]), { from, to, force, path: currentPath });
  ...
  async run() {
    ...
    const filePath = path.join(process.cwd(), "config/campaign", `${campaign}.json`);
    ...
    try {
      ...
      this.log(`Translating campaign '${campaign}' from '${from}' to '${to}'...`);
      ...
      if (flags.namespace) {
        ...
        if (!sourceLocale) {
          this.error(`Namespace "${namespace}" not found in source locale "${from}"`);
        }
      ...
      await this.traverseAndTranslate(sourceLocale, targetLocale, { from, to, force });
