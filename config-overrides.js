...
//
const componentLoader = (id,portal=[],journey) =>{
  ...
  j.forEach( k => {
    ...
    script += "import "+k.replace('/','_')+" from '"+path.resolve(__dirname, 'src/components/')+'/'+Component+".js';\n";
  });

  const imported=[];
  portal.forEach( k => {
    if (k.component) {
      if (imported.includes(k.component)) return;
      console.log(imported,k.component);
      imported.push(k.component);
      script += "import "+k.component.replace('/','_')+" from '"+path.resolve(__dirname, 'src/components/')+'/'+k.component+".js';\n";
    } else {
      script += "import "+k.replace('/','_')+" from '"+path.resolve(__dirname, 'src/components/')+'/'+k+".js';\n";
    }
  });
  script += "let steps={};\n";
  script += "let portals={};\n";
  ...
  portal.forEach( k => {
    let name= k.component ? k.component.replace('/','_') : k.replace('/','_');
    script += "portals['"+name+"']=" + name +";\n";
  });
...
//  config = addReactRefresh({ overlay: {sockIntegration: 'whm' }}) (config,env);
  ...
  config.resolve.alias['ComponentLoader$']= componentLoader(widget.actionpage, widget.portal,widget.journey);
